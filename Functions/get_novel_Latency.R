novel_Latencies <- function(filename, tibble = FALSE) {
  ...
  # #filter dfs to only include instances of novel huddle by filtering out the instances of partner huddle
  for (i in 1:length(x)) {
    x[[i]] <- if(partner_sides[[i]] == "right") {
      x[[i]] %>% filter(Event == "Social Contact [ 1 with 2 ] in Area left while Joint Motion < 0.030")} else {
        x[[i]] %>% filter(Event == "Social Contact [ 1 with 3 ] in Area right while Joint Motion < 0.030")
      }
  }
  # 
